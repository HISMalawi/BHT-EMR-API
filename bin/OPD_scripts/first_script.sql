...
select p.person_id, p.gender, p.birthdate, p.death_date, pn.family_name, pn.given_name, pi.identifier, pi.identifier_type
from openmrs_area18_art.patient_identifier pi
 inner join openmrs_area18_art.person p on p.person_id = pi.patient_id
 inner join openmrs_area18_art.person_name pn on pn.person_id = p.person_id
where pi.voided = 0 and pi.identifier_type IN (3, 2) 
and pi.patient_id not in (select person_id from openmrs_area18_art.users) order by pi.patient_id;
...
FROM openmrs_area18_opd.opd_patient_details p
INNER JOIN openmrs_area18_art.art_patient_details a ON a.identifier = p.identifier
...
FROM openmrs_area18_opd.opd_patient_details_in_art a INNER JOIN openmrs_area18_opd.encounter e ON e.patient_id = a.OPD_patient_id WHERE e.voided = 0 
...
select e.encounter_id, e.encounter_type, e.patient_id, ART_user_id, e.location_id, e.form_id, e.encounter_datetime, u.ART_user_id, e.date_created, e.voided, e.voided_by, e.date_voided, e.void_reason, (select uuid()), e.changed_by, e.date_changed, 14 from openmrs_area18_opd.encounter_back e
inner join openmrs_area18_opd.user_backup u ON e.creator = u.OPD_user_id
group by e.patient_id, e.encounter_id, e.encounter_type, DATE(e.encounter_datetime);
...
FROM openmrs_area18_opd.drug_order d
inner join openmrs_area18_opd.orders_bak o on o.OPD_order_id = d.order_id GROUP BY o.OPD_order_id;
...
FROM openmrs_area18_opd.obs o 
inner join openmrs_area18_opd.opd_patient_details_in_art e on e.OPD_patient_id = o.person_id 
inner join openmrs_area18_opd.user_backup u on u.OPD_user_id = o.creator
...
INNER JOIN openmrs_area18_opd.opd_patient_details_in_art e on e.OPD_patient_id = p.patient_id 
INNER JOIN openmrs_area18_opd.user_backup u on u.OPD_user_id = p.creator GROUP BY p.patient_program_id;
...
SELECT p.ART_patient_program_id, f.state, f.start_date, f.end_date, p.creator, f.date_created, f.changed_by, f.date_changed, f.voided, f.voided_by, f.date_voided, f.void_reason, (SELECT UUID()) FROM  openmrs_area18_opd.patient_state f
INNER JOIN openmrs_area18_opd.patient_program_backup p on p.OPD_patient_program_id = f.patient_program_id
