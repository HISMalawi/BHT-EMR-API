...
DROP TABLE IF EXISTS openmrs_area18_opd.opd_duplicate_patients;

CREATE TABLE openmrs_area18_opd.opd_duplicate_patients AS
select patient_id AS OPD_person_id,  (SELECT @max_patient_id + patient_id) AS ART_person_id from openmrs_area18_opd.opd_patients_with_multiple_identifier;
...
FROM openmrs_area18_opd.person p 
 INNER JOIN openmrs_area18_opd.opd_duplicate_patients pp ON pp.OPD_person_id = p.person_id AND p.voided = 0
...
FROM openmrs_area18_opd.patient p 
	INNER JOIN openmrs_area18_opd.opd_duplicate_patients pp ON pp.OPD_person_id = p.patient_id AND p.voided = 0
...
FROM openmrs_area18_opd.encounter e 
	INNER JOIN openmrs_area18_opd.opd_duplicate_patients pp ON pp.OPD_person_id = e.patient_id and e.voided = 0
