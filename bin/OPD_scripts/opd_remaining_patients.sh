...
echo "migrating remaining patients"
...
DROP TABLE IF EXISTS $OPDDATABASE.opd_patients_with_multiple_identifier;

CREATE TABLE $OPDDATABASE.opd_patients_with_multiple_identifier AS
select patient_id, count(*) as num from $OPDDATABASE.patient_identifier where voided = 0 and identifier_type in (3) 
and identifier not in (select identifier from $DATABASE.patient_identifier where voided = 0) group by patient_id having num > 1;

/* dropping and creating person_back_up */
DROP TABLE IF EXISTS $OPDDATABASE.opd_remaining_patients;

CREATE TABLE $OPDDATABASE.opd_remaining_patients AS
select patient_id AS OPD_person_id,  (SELECT @max_patient_id + patient_id) AS ART_person_id from $OPDDATABASE.patient_identifier where voided = 0 
and identifier_type in (3) 
and identifier not in (select a.identifier from $DATABASE.patient_identifier a where a.voided = 0) 
and patient_id not in (select o.patient_id from $OPDDATABASE.opd_patients_with_multiple_identifier o);
...
FROM $OPDDATABASE.person p 
 INNER JOIN $OPDDATABASE.opd_remaining_patients pp ON pp.OPD_person_id = p.person_id AND p.voided = 0
...
FROM $OPDDATABASE.patient p 
  INNER JOIN $OPDDATABASE.opd_remaining_patients pp ON pp.OPD_person_id = p.patient_id AND p.voided = 0
...
FROM $OPDDATABASE.encounter e 
  INNER JOIN $OPDDATABASE.opd_remaining_patients pp ON pp.OPD_person_id = e.patient_id and e.voided = 0
...
echo "Finished migrating remaining patients"
