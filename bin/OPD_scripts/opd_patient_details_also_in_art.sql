...
from $DATABASE.patient_identifier pi  inner join $DATABASE.person p on p.person_id = pi.patient_id  inner join $DATABASE.person_name pn on pn.person_id = p.person_id where pi.voided = 0 and pi.identifier_type IN (3, 2) and pi.patient_id not in (select person_id from $DATABASE.users) order by pi.patient_id;
...
FROM $OPDDATABASE.opd_patient_details p INNER JOIN $DATABASE.art_patient_details a ON a.identifier = p.identifier WHERE (a.given_name = p.given_name AND a.family_name = p.family_name) AND (p.gender = a.gender AND p.birthdate = a.birthdate);
...
FROM $OPDDATABASE.opd_patient_details_in_art a INNER JOIN $OPDDATABASE.encounter e ON e.patient_id = a.OPD_patient_id WHERE e.voided = 0  group by e.patient_id, e.encounter_id, e.encounter_type, DATE(e.encounter_datetime);
...
SELECT o.ART_order_id, drug_inventory_id, dose, equivalent_daily_dose, units, frequency, prn, complex, quantity FROM $OPDDATABASE.drug_order d inner join $OPDDATABASE.orders_bak o on o.OPD_order_id = d.order_id GROUP BY o.OPD_order_id;
...
FROM $OPDDATABASE.obs o inner join $OPDDATABASE.opd_patient_details_in_art e on e.OPD_patient_id = o.person_id inner join $OPDDATABASE.user_backup u on u.OPD_user_id = o.creator WHERE o.voided = 0 group by o.obs_id;
...
FROM $OPDDATABASE.patient_program p INNER JOIN $OPDDATABASE.opd_patient_details_in_art e on e.OPD_patient_id = p.patient_id INNER JOIN $OPDDATABASE.user_backup u on u.OPD_user_id = p.creator GROUP BY p.patient_program_id;
...
SELECT p.ART_patient_program_id, f.state, f.start_date, f.end_date, p.creator, f.date_created, f.changed_by, f.date_changed, f.voided, f.voided_by, f.date_voided, f.void_reason, (SELECT UUID()) FROM  $OPDDATABASE.patient_state f INNER JOIN $OPDDATABASE.patient_program_backup p on p.OPD_patient_program_id = f.patient_program_id ORDER BY patient_program_id;
