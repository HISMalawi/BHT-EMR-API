...
DROP table IF EXISTS $ANCDATABASE.patients_remaining_to_be_migrated;

create table $ANCDATABASE.patients_remaining_to_be_migrated as
SELECT ANC_patient_id, (SELECT @max_patient_id + ANC_patient_id) AS ART_patient_id, ART_gender, new_identifier_type, ART_creator
FROM $ANCDATABASE.anc_art_patients_with_voided_art_identifier WHERE per_voided = 0;
...
from $ANCDATABASE.person p 
 inner join $ANCDATABASE.patients_remaining_to_be_migrated pp ON pp.ANC_patient_id = p.person_id and p.voided = 0
...
from $ANCDATABASE.patient p 
	inner join $ANCDATABASE.patients_remaining_to_be_migrated pp ON pp.ANC_patient_id = p.patient_id and p.voided = 0
	left join $ANCDATABASE.user_bak c on c.ANC_user_id = p.creator
	left join $ANCDATABASE.user_bak u on u.ANC_user_id = p.changed_by;
...
select pp.ART_patient_id, p.identifier, pp.new_identifier_type,  p.preferred,  p.location_id,  c.ART_user_id,  p.date_created,  p.voided,  p.voided_by, p.date_voided, p.void_reason, (select uuid()) 
...
from $ANCDATABASE.encounter e 
	inner join $ANCDATABASE.patients_remaining_to_be_migrated pp ON pp.ANC_patient_id = e.patient_id and e.voided = 0
