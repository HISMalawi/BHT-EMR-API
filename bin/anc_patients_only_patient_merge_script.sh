...
select (select @max_patient_id + pp.patient_id) as patient_id, tribe, creator, date_created,   changed_by, date_changed, voided,  voided_by, date_voided, void_reason from $ANCDATABASE.patient p inner join $ANCDATABASE.ANC_only_patients_details pp on pp.patient_id = p.patient_id and p.voided = 0;
...
select (select @max_patient_id + pp.patient_id) as patient_id, identifier,  identifier_type,  preferred,  location_id,  creator,  date_created,  voided,   voided_by,  date_voided,  void_reason, (select uuid()) from $ANCDATABASE.patient_identifier p inner join $ANCDATABASE.ANC_only_patients_details pp on pp.patient_id = p.patient_id and p.voided = 0;
...
SELECT ob.ART_order_id, dr.drug_inventory_id, dr.dose, dr.equivalent_daily_dose, dr.units, dr.frequency, dr.prn, dr.complex, dr.quantity FROM $ANCDATABASE.drug_order dr INNER JOIN $ANCDATABASE.orders_bak ob on dr.order_id = ob.ANC_order_id;
