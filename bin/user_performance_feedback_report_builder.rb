...
def notification_tracker_user_activities
  ...
  activities.each do |a|
    ...
    @selected_activities[user_id] = {} if @selected_activities[user_id].blank?

    @selected_activities[user_id][login_datetime] = [] if @selected_activities[user_id][login_datetime].blank?
...
def fetch_overall_record_complete_status
  ...
  (all_patients || []).each do |r|
    ...
    next if skipped_encounter_type.name.blank?
...
def check_for_skipped_encounter(patient_id, visit_date)
  ...
  check_visit_completeness(patient_id)
end
...
def check_visit_completeness(patient_id, check_all_encounters = false)
  ...
  ############## no.3 Vitals ##################################
  if !fast_track_patient && patient_visit
  ...
  patient_arv_prescriptions(Patient.find(patient_id), @start_date.to_date).each do |_order|
    arv_drugs_given = true
    break
  end

  if arv_drugs_given && adherence_encounter_available.blank?
    all_encounters_missed << encounter_type
    return encounter_type unless check_all_encounters
  end
  ####################################################################
  ...
  return all_encounters_missed if check_all_encounters
...
def build_individual_record_complete_status
  ...
  (provider_patient_interactions || []).each do |i|
    ...
    next if user_activities_id.blank?
