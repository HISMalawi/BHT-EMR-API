...
class Visit < VoidableRecord
  ...
  def as_json(options = {})
    super(options.merge(methods: %i[visit_type_name patient]))
  end
  ...
  def validate_dates
    ...
    if date_started && date_started.strftime('%H:%M:%S') == '00:00:00'
      errors.add(:date_started,
                 'should contain time')
    end

    # check if date_stoped has a valid date and time instead of zeros for time
    if date_stopped && date_stopped.strftime('%H:%M:%S') == '00:00:00'
      errors.add(:date_stopped,
                 'should contain time')
    end
