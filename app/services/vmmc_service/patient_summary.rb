    ...
    def vitals_temperature
      temperature = Observation.joins([:encounter])
                               .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                                       ...
                                       ConceptName.find_by_name('Temperature (c)').concept_id]).order(:obs_datetime).last&.value_numeric
    end
    ...
    def systolic_blood_pressure
      sbp = Observation.joins([:encounter])
                       .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                               patient.id,
                               EncounterType.find_by_name('Vitals').id,
                               ConceptName.find_by_name('Systolic blood pressure').concept_id]).order(:obs_datetime).last&.value_numeric.to_i
    end
    ...
    def diastolic_blood_pressure
      dbp = Observation.joins([:encounter])
                       .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                               patient.id,
                               EncounterType.find_by_name('Vitals').id,
                               ConceptName.find_by_name('Diastolic blood pressure').concept_id]).order(:obs_datetime).last&.value_numeric.to_i
    end
    ...
    def vitals_weight
      weight = Observation.joins([:encounter])
                          .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                                  patient.id,
                                  EncounterType.find_by_name('Vitals').id,
                                  ConceptName.find_by_name('Weight (kg)').concept_id]).order(:obs_datetime).last&.value_numeric
    end
    ...
    def vitals_pulse
      pulse = Observation.joins([:encounter])
                         .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                                 patient.id,
                                 EncounterType.find_by_name('Vitals').id,
                                 ConceptName.find_by_name('Pulse').concept_id]).order(:obs_datetime).last&.value_numeric
    end
    ...
    def postop_spo
      postop_spo = Observation.joins([:encounter])
                              .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                                      patient.id,
                                      EncounterType.find_by_name('Post-op review').id,
                                      ConceptName.find_by_name('Blood oxygen saturation').concept_id]).order(:obs_datetime).last&.value_numeric
    end
    ...
    def vitals_bmi
      bmi = Observation.joins([:encounter])
                       .where(['person_id = ? AND encounter_type = ? AND obs.concept_id = ?',
                               patient.id,
                               EncounterType.find_by_name('Vitals').id,
                               ConceptName.find_by_name('Body mass index, measured').concept_id]).order(:obs_datetime).last&.value_numeric
    end
  end
end
