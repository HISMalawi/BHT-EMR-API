class AetcService::Reports::Clinic::MalariaReport
  ...
  def build_malaria_hash
...

    {
      ...
      presumed_malaria_LA_less_5yrs: get_ids('< 5yrs','presume','Lumefantrine')-get_ids('< 5yrs','confrim','Lumefantrine'),
      presumed_malaria_LA_more_5yrs: get_ids('> 5yrs','presume','Lumefantrine')-get_ids('> 5yrs','confrim','Lumefantrine'),
      presumed_malaria_ASAQ_less_5yrs: get_ids('< 5yrs','presume','ASAQ')-get_ids('< 5yrs','confrim','ASAQ'),
      presumed_malaria_ASAQ_more_5yrs: get_ids('> 5yrs','presume','ASAQ')-get_ids('> 5yrs','confrim','ASAQ'),
  ...
  def get_ids(age_range,condition_name,drug_name)
    array_id = []
      ...
      @malaria_data.select { |element|
        if (element.match?(age_range) && element.match?(condition_name) && drug_name == "") ||
          (element.match?(age_range) && element.match?(condition_name) && element.gsub(/[()]/, '').match?(drug_name) && drug_name != "" && age_range != "") ||
