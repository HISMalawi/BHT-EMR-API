  ...
  module AitIntergration
    class AitIntergrationService
      ...
      def initialize(patient_id)
        if AIT_CONFIG['endpoint'].empty?
          raise 'AIT config not found or not properly set, please refer to the ait.yml.example'
        end
      ...
      def sync
        ...
        index.map do |obj|
          obj[:contacts] = contacts.select do |contact|
            contact['parent_external_id'] == obj[:client_patient_id]
          end
          obj
        end
      end
      ...
      def create_contacts_rows(index)
        ...
        get_index_contacts(index).collect do |contact|
          CONTACT_ADDITIONAL_HEADERS.each do |header|
            ...
            contact['client_patient_id'] = "#{index.id}_#{contact['first_name']}_#{contact['last_name']}"
          end
          contact
        end
      end
