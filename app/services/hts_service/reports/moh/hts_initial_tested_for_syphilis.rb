...
module HtsService
  module Reports
      ...
      class HtsInitialTestedForSyphilis
        ...
        def initialize(start_date:, end_date:)
          ...
          @data = {
                 ...
                 'total_clients_tested_for_syphilis'  => [],
                ...
                "time_since_last_taken_arvs_not_applicable_or_missing" => [],
                 ...
                 'referral_for_prevention_services_pep' => [],
                 'referral_for_prep' => [],
                'referral_for_pep' => [],
                'referral_for_vmmc' => [],
                'referral_for_sti' => [],
        ...
        def fetch_hepatitis_b_clients
          ...
          Person.joins("INNER JOIN encounter e ON e.patient_id = person.person_id AND e.encounter_type = #{EncounterType.find_by_name("TESTING").encounter_type_id} AND e.voided = 0 AND e.program_id = #{Program.find_by_name("HTC Program").program_id}")
                ...
                .joins("INNER JOIN obs o3 ON o3.person_id = e.patient_id AND o3.voided = 0 AND o3.concept_id = #{ConceptName.find_by_name('Syphilis Test Result').concept_id} AND e.encounter_id = o3.encounter_id")
                ...
                .each do |client|
                    ...
                    @data["total_clients_tested_for_syphilis"].push(client.person_id) if client.results != nil
                 ...
                 status = Observation.joins(:encounter)\
                   ...
                   values = Observation.joins(:encounter)\
                   .where(concept: concept('Pregnancy status'),
                           person: client.person_id,
                        encounter: { encounter_type: EncounterType.find_by_name("PREGNANCY STATUS").encounter_type_id,
                                         program_id: Program.find_by_name("HTC Program").program_id })\
                   .where("encounter_datetime BETWEEN '#{start_date}' AND '#{end_date}' + INTERVAL 1 DAY ")\
                   .last

                    unless values.blank?
                        @data['sex_or_pregnancy_female_nonpregnant'].push(client.person_id) if ConceptName.find_by_name('Not Pregnant / Breastfeeding').concept_id == values.concept_id
                        @data['sex_or_pregnancy_female_pregnant'].push(client.person_id) if ConceptName.find_by_name('Patient pregnant').concept_id == values.concept_id
                        @data['sex_or_pregnancy_female_breastfeeding'].push(client.person_id) if ConceptName.find_by_name('Breastfeeding').concept_id == values.concept_id
                    end
                 ...
                 Person.joins("INNER JOIN encounter e ON e.patient_id = person.person_id AND e.encounter_type = #{EncounterType.find_by_name("TESTING").encounter_type_id} AND e.voided = 0 AND e.program_id = #{Program.find_by_name("HTC Program").program_id}")
                       ...
                       .joins("LEFT JOIN obs o4 ON o4.person_id = e.patient_id AND o4.voided = 0 AND o4.concept_id = #{ConceptName.find_by_name('Hepatitis B Test Result').concept_id} AND e.encounter_id = o4.encounter_id")
                       .joins("INNER JOIN obs o5 ON o5.person_id = e.patient_id AND o5.voided = 0 AND o5.concept_id = #{ConceptName.find_by_name('Syphilis Test Result').concept_id} AND e.encounter_id = o5.encounter_id")
                       ...
                       .each do |client|
                        ...
                        case array[1].to_s
                          ...
                          @data["time_since_last_hiv_test_12plus_months"].push(client.person_id)
                ...
                obs = Observation.joins(:encounter)\
                                 .joins("INNER JOIN obs o5 ON o5.person_id = encounter.patient_id AND o5.voided = 0 AND o5.concept_id = #{ConceptName.find_by_name('Syphilis Test Result').concept_id} AND encounter.encounter_id = o5.encounter_id")
                                 .where(concept: concept('Antiretroviral medication history'),
                                         person: client.person_id,
                ...
                unless obs.blank?
                  ...
                  if ConceptName.find_by_name('No').concept_id == obs.value_coded

                        @data["ever_taken_arvs_no"].push(obs.person_id) if !@data["ever_taken_arvs_no"].include?obs.person_id

                  elsif ConceptName.find_by_name('Yes').concept_id == obs.value_coded

                        Observation.where(encounter_id:obs.encounter_id).each do |drug|

                              if ConceptName.find_by_name("Given drugs").concept_id == drug.concept_id

                                   @data["ever_taken_arvs_prep"].push(obs.person_id) if ConceptName.find_by_name('Prep or infant NVP').concept_id == drug.value_coded
                                   @data["ever_taken_arvs_pep"].push(obs.person_id) if ConceptName.find_by_name('PEP').concept_id == drug.value_coded
                                   @data["ever_taken_arvs_art"].push(obs.person_id) if ConceptName.find_by_name('ARV').concept_id == drug.value_coded
                              end
                              if ConceptName.find_by_name("Time since last taken medication").concept_id == drug.concept_id
                                   ...
                                   case array[1].to_s
                                        ...
                                        when "Days"
                                             ...
                                             @data["time_since_last_taken_arvs_same_day"].push(obs.person_id) if array[0].to_i == 1
                                             @data["time_since_last_taken_arvs_1_to_13_days"].push(obs.person_id) if array[0].to_i > 1 && array[0].to_i < 14
                                             @data["time_since_last_taken_arvs_14_days_to_2_months"].push(obs.person_id) if array[0].to_i > 13 && array[0].to_i < 61
                                             @data["time_since_last_taken_arvs_3_to_5_months"].push(obs.person_id) if array[0].to_i > 60 && array[0].to_i < 150
                                             @data["time_since_last_taken_arvs_6_to_11_months"].push(obs.person_id) if array[0].to_i > 149 && array[0].to_i < 330
                                             @data["time_since_last_taken_arvs_12_plus_months"].push(obs.person_id) if array[0].to_i > 329

                                        when "Months"

                                            @data["time_since_last_taken_arvs_14_days_to_2_months"].push(obs.person_id) if array[0].to_i < 3
                                            @data["time_since_last_taken_arvs_3_to_5_months"].push(obs.person_id) if array[0].to_i > 2 && array[0].to_i < 6
                                            @data["time_since_last_taken_arvs_6_to_11_months"].push(obs.person_id) if array[0].to_i > 5 && array[0].to_i < 12
                                            @data["time_since_last_taken_arvs_12_plus_months"].push(obs.person_id) if array[0].to_i > 11

                                        when "Years"
                                          @data["time_since_last_taken_arvs_12_plus_months"].push(obs.person_id)
                                       end
                              end
                        end
                  end
    ...
    Person.joins("INNER JOIN encounter e ON e.patient_id = person.person_id AND e.encounter_type = #{EncounterType.find_by_name("TESTING").encounter_type_id} AND e.voided = 0 AND e.program_id = #{Program.find_by_name("HTC Program").program_id}")
          ...
          .joins("INNER JOIN obs o5 ON o5.person_id = e.patient_id AND o5.voided = 0 AND o5.concept_id = #{ConceptName.find_by_name('Syphilis Test Result').concept_id} AND e.encounter_id = o5.encounter_id")
          .select("person.person_id person_id,obs.concept_id concept_id,obs.value_coded value_coded")
          .where("person.voided = 0 AND person.person_id = '#{client.person_id}' AND DATE(e.encounter_datetime) BETWEEN '#{start_date}' AND '#{end_date}' + INTERVAL 1 DAY")
          .each do |tests|

           if ConceptName.find_by_name("Test 1").concept_id == tests.concept_id

              @data["hiv_test_1_result_negative"].push(tests.person_id) if ConceptName.find_by_name('Negative').concept_id == tests.value_coded
              @data["hiv_test_1_result_positive"].push(tests.person_id) if ConceptName.find_by_name('Positive').concept_id == tests.value_coded
          ...
          if ConceptName.find_by_name('Hepatitis B Test Result').concept_id == tests.concept_id
            ...
            @data["hepatitis_b_test_result_negative"].push(tests.person_id) if ConceptName.find_by_name('Negative').concept_id == tests.value_coded
            @data["hepatitis_b_test_result_positive"].push(tests.person_id) if ConceptName.find_by_name('Positive').concept_id == tests.value_coded

           end

           if ConceptName.find_by_name('Syphilis Test Result').concept_id == tests.concept_id

             @data["syphilis_test_result_negative"].push(tests.person_id) if ConceptName.find_by_name('Negative').concept_id == tests.value_coded
             @data["syphilis_test_result_positive"].push(tests.person_id) if ConceptName.find_by_name('Positive').concept_id == tests.value_coded
        ...
        def fetch_items_given
            ...
            .joins("INNER JOIN obs o4 ON o4.person_id = patient.patient_id AND o4.voided = 0 AND o4.concept_id = #{ConceptName.find_by_name('Syphilis Test Result').concept_id} AND encounter.encounter_id = o4.encounter_id")
            .joins(<<~SQL)
