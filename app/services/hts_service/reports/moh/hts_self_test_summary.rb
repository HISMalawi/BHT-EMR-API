  ...
  module Reports
    module Moh
      class HtsSelfTestSummary
        ...
        LAST_HIV_RESULT_DATE = %i[last_hiv_result_same_day last_hiv_result_12_plus_months last_hiv_result_1_to_13_days
                                  last_hiv_result_6_to_11_months last_hiv_result_3_to_5_months last_hiv_result_14_days_to_2_months last_hiv_result_less_than_14_days].freeze
        ...
        def calc_time_since_last_hiv_result(clients)
          ...
          clients.where(obs: { concept_id: ConceptName.find_by_name('Time of HIV test').concept_id })
            ...
            if client.value_datetime <= 0.days.ago && client.value_datetime >= 1.day.ago
              report[:last_hiv_result_same_day].push(client.patient_id)
            end
            if client.value_datetime.to_date <= 12.months.ago
              report[:last_hiv_result_12_plus_months].push(client.patient_id)
            end
            if client.value_datetime <= 6.months.ago && client.value_datetime >= 11.months.ago
              report[:last_hiv_result_6_to_11_months].push(client.patient_id)
            end
            if client.value_datetime <= 3.months.ago && client.value_datetime >= 5.months.ago
              report[:last_hiv_result_3_to_5_months].push(client.patient_id)
            end
            if client.value_datetime <= 14.days.ago && client.value_datetime >= 2.months.ago
              report[:last_hiv_result_14_days_to_2_months].push(client.patient_id)
            end
            if client.value_datetime <= 1.day.ago && client.value_datetime >= 13.days.ago
              report[:last_hiv_result_1_to_13_days].push(client.patient_id)
            end
          end
        end
