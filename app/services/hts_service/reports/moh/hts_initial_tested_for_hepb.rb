  ...
  module Reports
    module Moh
      ...
      class HtsInitialTestedForHepb
        ...
        INDICATORS = [
          { name: 'hiv_status', concept_id: ConceptName.find_by_name('HIV status').concept_id, value: 'value_coded', join: 'LEFT' },
          { name: 'access_type', concept_id: ConceptName.find_by_name('HTS Access Type').concept_id, value: 'value_coded', join: 'LEFT' },
          { name: 'test_location', concept_id: ConceptName.find_by_name('Location where test took place').concept_id, value: 'value_text', join: 'LEFT' },
          { name: 'hep_b_test_result', concept_id: ConceptName.find_by_name(HEP_B_TEST_RESULT).concept_id, value: 'value_coded', join: 'INNER' },
          { name: 'syphilis_test_result', concept_id: ConceptName.find_by_name(SYPHILIS_TEST_RESULT).concept_id, value: 'value_coded', join: 'LEFT' },
          {
            name: %w[test_one test_two test_three],
            concept_id: [ConceptName.find_by_name(TEST_ONE).concept_id, ConceptName.find_by_name(TEST_TWO).concept_id, ConceptName.find_by_name(TEST_THREE).concept_id],
            join: 'LEFT',
          },
          { name: 'pregnancy_status', concept_id: ConceptName.find_by_name(PREGNANCY_STATUS).concept_id, value: 'value_coded', join: 'LEFT' },
          { name: 'circumcision_status', concept_id: ConceptName.find_by_name(CIRCUMCISION_STATUS).concept_id, value: 'value_coded', join: 'LEFT' },
          { name: 'male_condoms', concept_id: ConceptName.find_by_name(MALE_CONDOMS).concept_id, join: 'LEFT', value: 'value_numeric' },
          { name: 'female_condoms', concept_id: ConceptName.find_by_name(FEMALE_CONDOMS).concept_id, join: 'LEFT', value: 'value_numeric' },
          { name: 'frs', concept_id: ConceptName.find_by_name(FRS).concept_id, join: 'LEFT', value: 'value_numeric' },
          { name: 'referal_for_retesting', concept_id: ConceptName.find_by_name(REFERRAL_FOR_RETESTING).concept_id, join: 'LEFT' },
          { name: 'time_of_hiv_test', concept_id: ConceptName.find_by_name(TIME_OF_HIV_TEST).concept_id, value: 'value_datetime', join: 'LEFT' },
          { name: 'time_since_last_medication', value: 'value_datetime', concept_id: ConceptName.find_by_name(TIME_SINCE_LAST_MEDICATION).concept_id, join: 'LEFT' },
          { name: 'previous_hiv_test', concept_id: ConceptName.find_by_name(PREVIOUS_HIV_TEST).concept_id, join: 'LEFT' },
          { name: 'previous_hiv_test_done', concept_id: ConceptName.find_by_name(PREVIOUS_HIV_TEST_DONE).concept_id, join: 'LEFT' },
          { name: 'risk_category', concept_id: ConceptName.find_by_name(RISK_CATEGORY).concept_id, join: 'LEFT' },
          { name: 'partner_present', concept_id: ConceptName.find_by_name(PARTNER_PRESENT).concept_id, value: 'value_text', join: 'LEFT' },
          { name: 'partner_hiv_status', concept_id: ConceptName.find_by_name(PARTNER_HIV_STATUS).concept_id, join: 'LEFT' },
          { name: 'taken_arvs_before', concept_id: ConceptName.find_by_name(TAKEN_ARVS_BEFORE).concept_id, join: 'LEFT' },
          { name: 'taken_prep_before', concept_id: ConceptName.find_by_name(TAKEN_PREP_BEFORE).concept_id, join: 'LEFT' },
          { name: 'taken_pep_before', concept_id: ConceptName.find_by_name(TAKEN_PEP_BEFORE).concept_id, join: 'LEFT' },
          { name: 'referrals_ordered', concept_id: ConceptName.find_by_name(REFERALS_ORDERED).concept_id, value: 'value_text', join: 'LEFT' },
        ].freeze
        ...
        def fetch_hepatitis_b_clients
          ...
          @data['access_point_type_facility_vct'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'VCT'
          }
          @data['access_point_type_facility_anc_first_visit'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'ANC First visit'
          }
          @data['access_point_type_facility_inpatient'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'Inpatient'
          }
          @data['access_point_type_facility_sti'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'STI'
          }
          @data['access_point_type_facility_pmtctfup'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'PMTCT FUP'
          }
          @data['access_point_type_facility_index'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'Index'
          }
          @data['access_point_type_facility_paediatric'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'Paediatric'
          }
          @data['access_point_type_facility_malnutrition'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'Malnutrition'
          }
          @data['access_point_type_facility_vmmc'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'VMMC'
          }
          @data['access_point_type_facility_tb'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'TB'
          }
          @data['access_point_type_facility_opd'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'OPD'
          }
          @data['access_point_type_facility_other_pitc'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'Other PITC'
          }
          @data['access_point_type_facility_sns'] = filter_hash('access_type', concept('Health facility').concept_id).select { |q|
            q['test_location'] == 'SNS'
          }
          @data['access_point_type_community_vmmc'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'VMMC'
          }
          @data['access_point_type_community_index'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'Index'
          }
          @data['access_point_type_community_mobile'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'Mobile'
          }
          @data['access_point_type_community_vct'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'VCT'
          }
          @data['access_point_type_community_other'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'Other'
          }
          @data['access_point_type_community_sns'] = filter_hash('access_type', concept('Community').concept_id).select { |q|
            q['test_location'] == 'SNS'
          }
        ...
        def fetch_hiv_tests
          ...
          @data['last_hiv_test_positive_prof_test'] = filter_hash('previous_hiv_test_done', concept('Professional').concept_id).select { |q|
            [concept('Positive').concept_id, concept('Positive NOT on ART').concept_id, concept('Positive on ART').concept_id].include?(q['previous_hiv_test'])
          }
          @data['last_hiv_test_positive_prof_initial_test'] = filter_hash('previous_hiv_test_done', concept('Initial professional').concept_id).select { |q|
            [concept('Positive').concept_id, concept('Positive NOT on ART').concept_id, concept('Positive on ART').concept_id].include?(q['previous_hiv_test'])
          }
          ...
          @data['time_since_last_hiv_test_same_day'] = @query.select { |q|
            get_diff(q['encounter_datetime'], q['time_of_hiv_test']) == 0
          }
          @data['time_since_last_hiv_test_1_to_13_days'] = @query.select { |q|
            (1..13).include?(get_diff(q['encounter_datetime'], q['time_of_hiv_test']))
          }
          @data['time_since_last_hiv_test_35_months'] = @query.select { |q|
            (61..150).include?(get_diff(q['encounter_datetime'], q['time_of_hiv_test']))
          }
          @data['time_since_last_hiv_test_611_months'] = @query.select { |q|
            (151..330).include?(get_diff(q['encounter_datetime'], q['time_of_hiv_test']))
          }
          @data['time_since_last_hiv_test_14_days_to_2_months'] = @query.select { |q|
            (331..1000).include?(get_diff(q['encounter_datetime'], q['time_of_hiv_test']))
          }
          @data['time_since_last_hiv_test_12plus_months'] = @query.select { |q|
        ...
        def fetch_ever_taken_drugs_before
          ...
          @data['time_since_last_taken_arvs_1_to_13_days'] = @query.select { |q|
            (1..13).include?(get_diff(q['encounter_datetime'], q['time_since_last_medication']))
          }
          @data['time_since_last_taken_arvs_14_days_to_2_months'] = @query.select { |q|
            (14..60).include?(get_diff(q['encounter_datetime'], q['time_since_last_medication']))
          }
          @data['time_since_last_taken_arvs_35_months'] = @query.select { |q|
            (61..150).include?(get_diff(q['encounter_datetime'], q['time_since_last_medication']))
          }
          @data['time_since_last_taken_arvs_611_months'] = @query.select { |q|
            (151..330).include?(get_diff(q['encounter_datetime'], q['time_since_last_medication']))
          }
          @data['time_since_last_taken_arvs_12plus_months'] = @query.select { |q|
