      ...
      class HtsLegacyRegular # rubocop:disable Metrics/ClassLength
        ...
        def init_report
          ...
          report.keys.each do |key|
            ...
            report["community_#{key}"] = data.select { |q| q['access_type'] == 'Community' }.map { |q| q['person_id'] }.uniq
            report["facility_#{key}"] = data.select { |q| q['access_type'] == 'Health facility' }.map { |q| q['person_id'] }.uniq
          end
          report
        end
        ...
        def access_type_and_age_group
          ...
          report.merge!({
                          ...
                          other: data.reject { |q| ['Other PITC', 'FRS'].include?(q['test_location']) },
