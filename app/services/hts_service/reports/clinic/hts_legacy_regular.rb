module HtsService::Reports::Clinic
  class HtsLegacyRegular
    ...
    def access_type_and_age_group
      data = Person.connection.select_all(
        query.joins(
          ...
          "
        ).select("test_location.value_text as test_location, person.birthdate, p_status.value_coded as status")
      ...
      report[:fnp] = data.select { |q| [concept("Not Pregnant / Breastfeeding").concept_id, concept("Breastfeeding").concept_id].include?(q["status"]) }
      report[:fp] = data.select { |q| q["status"] == concept("Pregnant woman").concept_id }
    end
    ...
    def query
      ...
      @regular_clients.call(data).select("access_type_name.name as access_type, person.person_id, person.gender, prev_test.value_coded")
    end
  end
end
