        ...
        module Pepfar
            class TbStatArt
                ...
                def find_report
                    ...
                    on_arvs = concept('On ARVs').concept_id
                   ...
                   query.each do |row|
                        ...
                        arv_start_date = row['arv_start_date']
                        hiv_test_date = row['hiv_test_date']
                        enrollment_date = row['enrollment_date']
                        time_of_arv_test = row['time_of_arv_test']

                        @report[age_group][gender][:known_positive] << row['patient_id'] if hiv_status == positive && arv_status == before_tb
                        @report[age_group][gender][:newly_tested_positive] << row['patient_id'] if hiv_status == positive && arv_status == while_tb
                        @report[age_group][gender][:new_negative] << row['patient_id'] if hiv_status == negative && arv_status == hiv_test_date&.to_date >= enrollment_date.to_date
                        @report[age_group][gender][:recent_negative] << row['patient_id'] if hiv_status == negative && arv_status == hiv_test_date&.to_date < 2.weeks.ago.to_date
                        @report[age_group][gender][:not_done] << row['patient_id'] if hiv_status == nil
                        @report[age_group][gender][:new_on_art] << row['patient_id'] if hiv_status == positive && time_of_arv_test&.to_date >= start_date && time_of_arv_test&.to_date <= end_date
                        @report[age_group][gender][:already_on_art] << row['patient_id'] if hiv_status == positive && time_of_arv_test&.to_date < end_date
                   end
