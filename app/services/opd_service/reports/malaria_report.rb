...
module OpdService
  module Reports
    class MalariaReport
      def find_report(start_date:, end_date:, site_id:, **_extra_kwargs)
      ...
      AND obs.concept_id = '#{visit_type.concept_id}' AND encounter.site_id = '#{@site_id}'"))\
      ...
      def malaria_report
        @malaria_data = Observation.where(Arel.sql("obs_datetime BETWEEN '#{@start_date.to_date.strftime('%Y-%m-%d 00:00:00')}' AND '#{@end_date.to_date.strftime('%Y-%m-%d 23:59:59')}'  AND c.voided = 0 AND obs.site_id = '#{@site_id}' AND c.name IN ('Amount dispensed', 'MRDT', 'Malaria film', 'Malaria Species', 'Primary diagnosis') AND
