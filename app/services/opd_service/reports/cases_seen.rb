  ...
  module Reports
    class CasesSeen
      ...
      def cases_seen(start_date, end_date)
        ...
        Encounter.where("
          encounter_datetime BETWEEN #{start_date.to_date.strftime('%Y-%m-%d 00:00:00')} AND #{end_date.to_date.strftime('%Y-%m-%d 23:59:59')}
          AND encounter_type = #{type.id}
          AND c.name IN(#{concept_names})
