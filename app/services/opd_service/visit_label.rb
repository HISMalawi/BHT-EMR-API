  ...
  class VisitLabel
    ...
    def filter_drug_instraction(o)
      ...
      o.each do |x|
        ...
        drug_name = drug_name.truncate(27) if drug_name.length > 30
        ...
        drug_duration = '1day' if drug_duration == '1days'
    ...
    def print
      ...
      title_header_font = { font_reverse: false, font_size: 4, font_horizontal_multiplier: 1,
                            font_vertical_multiplier: 1 }
      concepts_font = { font_reverse: false, font_size: 3, font_horizontal_multiplier: 1,
                        font_vertical_multiplier: 1 }
      title_font_top_bottom = { font_reverse: false, font_size: 4, font_horizontal_multiplier: 1,
                                font_vertical_multiplier: 1 }
      title_font_bottom = { font_reverse: false, font_size: 2, font_horizontal_multiplier: 1,
                            font_vertical_multiplier: 1 }
      ...
      encs.each do |encounter|
        if encounter.name.upcase.include?('TREATMENT')
          ...
          o = encounter.orders.collect do |order|
            order.drug_order.to_s if order.order_type_id == OrderType.find_by_name('Drug Order').order_type_id
          end.join("\n")
          ...
          label.draw_multi_text(o.to_s, concepts_font)

        elsif encounter.name.upcase.include?('PROCEDURES DONE')
          procs = ['Procedures - ']
          procs << encounter.observations.collect do |observation|
            observation.answer_string.squish unless observation.concept.fullname.match(/Workstation location/i)
          end.compact.join('; ')
          label.draw_multi_text(procs.to_s, concepts_font)

        elsif encounter.name.upcase.include?('UPDATE HIV STATUS')
          hiv_status = []
          encounter.observations.each do |observation|
            next unless observation.concept.fullname.match(/HIV STATUS/i)

            begin
              hiv_status << "HIV Status - #{observation.answer_string}"
            rescue StandardError
              ''
            end
          end
          label.draw_multi_text(hiv_status.to_s, font_reverse: false)
          ...
          encounter.observations.each do |observation|
            concept_name = observation.concept.fullname
            next if concept_name.match(/Workstation location/i)
            next unless observation.obs_group_id.blank?

            child_obs = Observation.where('obs_group_id = ?', observation.obs_id)

            if !child_obs.empty?
              text = "#{observation.answer_string} - "
              count = 0
              child_obs.each do |child_observation|
                text += ', ' if count.positive?
          ...
          obs.each do |observation|
            label.draw_multi_text(observation.to_s, concepts_font)
          end
        elsif encounter.name.upcase.include?('TRANSFER OUT')
          obs = ['Referred to facility - ']
          obs << encounter.observations.collect do |observe|
            if observe.concept.fullname.upcase.include?('TRANSFER')
              Location.find(observe.answer_string.to_s.squish).name
            end
          end.compact.join('; ')
          label.draw_multi_text('Transfer Out', font_reverse: true)
          label.draw_multi_text(obs.to_s, concepts_font)
          ...
          if check_vitals == false
            ...
            label.draw_multi_text(obs.join(',').to_s, concepts_font)
          end
          ...
          encounter.observations.each do |observation|
            ...
            if !child_obs.empty?
              ...
              if vital_signs.include?(concept_name)
                string << "#{observation.concept.fullname}:#{observation.answer_string}"
              end
              bp << observation.answer_string if concept_name.match(/TA/i)
              bp << observation.answer_string if concept_name.match(/DIASTOLIC/i)
              if !vital_signs.include?(concept_name) && !concept_name.match(/TA/i) && !concept_name.match(/DIASTOLIC/i)
                if !concept_name.match(/LIFE THREATENING CONDITION/i) && !concept_name.match(/TRIAGE CATEGORY/i)
                  obs << observation.answer_string.to_s
                end
                if concept_name.match(/LIFE THREATENING CONDITION/i)
                  complaints << "#{concept_name}:#{observation.answer_string}"
                end
          ...
          if check_notes == false
            ...
            label.draw_multi_text(string.join(',').to_s, concepts_font)
            # label.draw_multi_text("Presenting complaints", title_header_font)
            label.draw_multi_text("Presenting complaints\n #{obs.join(',')}", concepts_font) unless obs.blank?

            unless complaints.blank?
              complaints.each do |complaint|
                label.draw_multi_text(complaint.to_s, concepts_font)
              end
            end
      ...
      if (check_notes == true) && (check_vitals == true)
        ...
        label.draw_multi_text(combined.join(',').to_s, concepts_font)
        # combined.each { | value |
      ...
      ['OPD PROGRAM', 'IPD PROGRAM'].each do |program_name|
        ...
        state = begin
          patient.patient_programs.local.select do |p|
            p.program_id == program_id
          end.last.patient_states.last
        rescue StandardError
          nil
        end
        ...
        label.draw_multi_text("Outcomes : #{outcomes.uniq.join(',')}", concepts_font) unless outcomes.blank?
      end
      program_id = Program.find_by_name('OPD Program').program_id
      user_id = Encounter.where(patient_id: patient['patient_id'],
                                program_id: program_id).order(encounter_datetime: :desc).first.creator
    ...
    def current_treatment_encounter(patient, date, _force = false)
      type = EncounterType.find_by_name('TREATMENT')
      program_id = Program.find_by_name('OPD Program').program_id

      patient.encounters.find_by(
