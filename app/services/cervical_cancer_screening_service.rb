  ...
  def report(patient, session_date)
    ...
    unless latest_cryo_result.blank?
      ...
      if cryo_result_answer == 'LESION SIZE TOO BIG'
        ...
        screening_report[:lesion_size_too_big] = false if date_gone_lesion_size_was_big >= three_years
      end
    end
    ...
    unless latest_cervical_cancer_result.blank?
      ...
      if latest_cervical_cancer_result == 'NEGATIVE'
        ...
        if date_gone_in_days >= three_years
          ...
          if !next_via_referral_obs.blank? && next_via_referral_obs.answer_string.squish.casecmp('YES').zero?
      ...
      if !latest_via_referral_outcome.blank? && (latest_via_referral_outcome == 'NO CANCER')
    ...
    via_done_date_answer_string = begin
      patient.person.observations.recent(1).question('VIA DONE DATE').last.answer_string.squish.to_date
    rescue StandardError
      nil
    end
    ...
    via_referral_outcome_answers.each do |outcome|
      next unless terminal_referral_outcomes.include?(outcome)
