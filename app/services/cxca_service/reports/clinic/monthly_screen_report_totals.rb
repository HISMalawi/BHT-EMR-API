module CXCAService::Reports::Clinic
  class MonthlyScreenReportTotals
    include Utils
    ...
    def init_report
      query = fetch_query
      report["total_screened"] = query.map { |q| q["person_id"] }.uniq
      report["total_negative"] = query.select { |q| q["screening_results"].to_s.downcase.in?(CxCa_TX_OUTCOMES[:negative]) }.map { |q| q["person_id"] }.uniq
      report["total_via_plus_eligible_for_same_day_tx"] = query.select { |d| d["via_result"].to_s.downcase == "VIA positive" || d["screening_results"] == "VIA positive" && (d["tx_option"] == "Same day treatment") }.uniq
      report["total_positive"] = query.select { |q| q["screening_results"].to_s.downcase.in?(CxCa_TX_OUTCOMES[:positive]) }.map { |q| q["person_id"] }.uniq
      report["total_suspect_cancer"] = query.select { |q| q["screening_results"].to_s.downcase.in?(CxCa_TX_OUTCOMES[:suspected]) }.map { |q| q["person_id"] }.uniq
      report
    end
