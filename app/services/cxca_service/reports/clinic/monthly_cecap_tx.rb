  ...
  class MonthlyCecapTx
    ...
    def get_indicators(x, age_group)
      ...
      TREATMENTS.each do |treatment|
        ...
        via_patients = x.select { |d| d["via_result"].to_s.downcase == "VIA positive" || d["screening_results"] == "VIA positive" }
        groups["via_deffered"] = via_patients.select { |d| d["tx_option"] == "Postponed treatment" }.map { |t| t["person_id"] }.uniq
        groups["via_reffered"] = via_patients.select { |d| d["tx_option"] == "Referral" }.map { |t| t["person_id"] }.uniq
        groups["suspected_reffered"] = x.select { |d| d["tx_option"] == "Referral" && d["treatment"] == "Suspect cancer" }.map { |t| t["person_id"] }.uniq
