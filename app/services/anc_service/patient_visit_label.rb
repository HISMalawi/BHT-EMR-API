  ...
  class PatientVisitLabel
    ...
    def print1
      ...
      @patient.encounters.where(['(encounter_type = ? OR encounter_type = ?) AND encounter_datetime >= ? AND encounter_datetime <= ?',
        ...
        @drugs[e.encounter_datetime.strftime('%d/%b/%Y')] = {} unless @drugs[e.encounter_datetime.strftime('%d/%b/%Y')]
    ...
    def print2
      ...
      out.each do |key, element|
        ...
        begin
          unless @drugs[element].empty?
            oth = @drugs[element].map do |d, v|
      ...
      (1..(@obstetrics.length + 1)).each do |pos|
        ...
        @dea = (if @obstetrics[pos]
                  (if @obstetrics[pos]['AGE AT DEATH']
                     (if @obstetrics[pos]['AGE AT DEATH'].to_s.match(/\.[1-9]/)
                      end
