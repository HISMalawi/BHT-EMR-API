  ...
  class ANCMigration
    ...
    def migrate_person
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE person_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating person records', query: statement
    end
    ...
    def migrate_person_name
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE person_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating person_name records', query: statement
    end
    ...
    def migrate_person_address
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE p.person_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating person_address records', query: statement
    end
    ...
    def migrate_person_attribute
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE p.person_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating person_attribute records', query: statement
    end
    ...
    def migrate_patient
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE p.patient_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating patient records', query: statement
    end
    ...
    def migrate_patient_identifier
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE p.patient_id IN (#{@patient_not_in_use})" : ''}
      SQL
      central_hub message: 'Migrating patient_identifier records', query: statement
    end
    ...
    def migrate_patient_program
      statement = <<~SQL
        ...
        #{@database_reversed ? "WHERE patient_id IN (#{@patient_not_in_use})" : ''}
      SQL
    ...
    def migrate_patient_state
      statement = <<~SQL
        ...
        WHERE patient_program_id IN (SELECT patient_program_id FROM #{@database}.patient_program #{@database_reversed ? "WHERE patient_id IN (#{@patient_not_in_use})" : ''})
      SQL
    ...
    def migrate_obs
      statement = <<~SQL
        ...
        FROM #{@database}.obs
        WHERE encounter_id IN (SELECT encounter_id FROM #{@database}.encounter #{@database_reversed ? "WHERE patient_id IN (#{@patient_not_in_use})" : ''})
      SQL
    ...
    def migrate_drug_order
      statement = <<~SQL
        ...
        WHERE order_id IN (SELECT order_id FROM #{@database}.orders #{@database_reversed ? "WHERE patient_id IN (#{@patient_not_in_use})" : ''})
      SQL
