  ...
  class ANCMissedMigration
    ...
    def migrate_orders(list)
      statement = <<~SQL
        ...
        SELECT (SELECT #{@order_id} + order_id) AS order_id,  order_type_id, concept_id, orderer, (SELECT #{@encounter_id} + encounter_id) AS encounter_id,  instructions, start_date, auto_expire_date,  discontinued,  discontinued_date, changers.ART_user_id,  discontinued_reason, creators.ART_user_id,  date_created,  voided, voiders.ART_user_id,  date_voided, void_reason, #{@patient}, accession_number, (SELECT #{@obs_id} + obs_id) AS obs_id, uuid, discontinued_reason_non_coded
    ...
    def migrate_patient_program(list)
      statement = <<~SQL
        ...
        SELECT (SELECT #{@patient_program_id} + patient_program_id) AS patient_program_id,  #{@patient},  program_id,  date_enrolled,  date_completed, creators.ART_user_id,  date_created, changers.ART_user_id, date_changed,  voided,  voiders.ART_user_id,  date_voided,  void_reason,  uuid, location_id
    ...
    def migrate_encounter(list)
      statement = <<~SQL
        ...
        SELECT (SELECT #{@encounter_id} + encounter_id) AS id, encounter_type, #{@patient}, providers.ART_person_id, location_id, form_id, encounter_datetime, creators.ART_user_id, date_created, voided, voiders.ART_user_id, date_voided, void_reason, uuid, changers.ART_user_id, date_changed, 12
        ...
        INNER JOIN user_bak creators ON creators.ANC_user_id = encounter.creator
        LEFT JOIN user_bak changers ON changers.ANC_user_id = encounter.changed_by
        LEFT JOIN user_bak voiders ON voiders.ANC_user_id = encounter.voided_by
