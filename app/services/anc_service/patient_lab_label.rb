    ...
    def print
      ...
      @patient.encounters.where(['encounter_type IN (?) AND program_id = ?',
        ...
        e.observations.each do |o|
          ...
          next unless ['PREVIOUS HIV TEST RESULTS',
                       'HIV STATUS'].include?(concept_name)
      ...
      @patient.encounters.where(['encounter_type = ?',
                                 EncounterType.find_by_name('LAB RESULTS').id]).order('encounter_datetime DESC').each do |e|
        e.observations.each do |o|
          hb["#{o.concept.concept_names.map(& :name).last.upcase} #{pos}"] = o.answer_string.squish.upcase
          pos += 1 if o.concept.concept_names.map(& :name).last.upcase == 'HB TEST RESULT DATE'
        end
      end

      @hb = begin
        "#{syphil['HB TEST RESULT']} g/dl"
      rescue StandardError
        nil
      end
      ...
      label.draw_text((@hb.blank? ? 'N/A' : @hb.to_s).to_s, 325, 206, 0, 2, 1, 1, false)
