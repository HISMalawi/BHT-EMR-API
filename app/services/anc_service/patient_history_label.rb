...
module AncService
  class PatientHistoryLabel
    ...
    def print
      ...
      still_births_concepts = ConceptName.find_by_sql("SELECT concept_id FROM concept_name where name like '%still birth%'").collect { |c|
        c.concept_id
      }.compact
      ...
      @vacuum = Observation.where(["person_id = ? AND (value_coded = ? OR value_text = ?)", @patient.id,
                                   ConceptName.find_by_name('Vacuum extraction delivery').concept_id, "Vacuum extraction delivery"]).length rescue nil
      ...
      label.draw_text("#{@haemorrhage}", 280, 239, 0, 2, 1, 1,
                      (((@haemorrhage.upcase == "PPH") rescue false) ? true : false))
    ...
    def active_range(date = Date.today)
      ...
      current_range["END"] =
        current_range["START"] + 7.day + 45.week unless ((current_range["START"]).to_date.blank? rescue true)
