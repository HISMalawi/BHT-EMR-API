...
ELSEIF age_in_years >= 50 THEN SET age_group = ">= 50 years";
END IF;
...
EOF
    ...
    def get_age_groups(age_group, start_date, end_date)
      if age_group != 'Pregnant' && age_group != 'Not pregnant' && age_group != 'Breastfeeding'
        tmp = ActiveRecord::Base.connection.select_all <<EOF
          SELECT 
...
EOF
        ...
        concept_id = ConceptName.find_by_name("Is patient pregnant?").concept_id
        value_coded = ConceptName.find_by_name("Yes").concept_id

        results = ActiveRecord::Base.connection.select_all <<EOF
          SELECT e.*, ob.* FROM obs ob
          INNER JOIN temp_earliest_start_date e ON e.patient_id = ob.person_id
          INNER JOIN temp_patient_outcomes o ON e.patient_id = o.patient_id
          WHERE e.gender = 'F' AND o.cum_outcome = 'On antiretrovirals'
          AND ob.obs_datetime = (
            SELECT MAX(t.obs_datetime) FROM obs t
            INNER JOIN encounter enc ON enc.patient_id = t.person_id
            AND t.obs_datetime <= '#{end_date.to_date.strftime('%Y-%m-%d 23:59:59')}' 
            AND t.concept_id = #{concept_id} AND t.value_coded = #{value_coded}
            AND t.voided = 0 AND enc.encounter_type = #{encounter_type}
            AND t.person_id = ob.person_id) GROUP BY ob.person_id;
EOF
